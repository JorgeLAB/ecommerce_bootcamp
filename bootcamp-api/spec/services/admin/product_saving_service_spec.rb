		...
		context "sending loaded product" do
			...
			context "with invalid :product params" do
				...
				it "raises NotSavedProductError" do
					expect{
						...
						service.call
					}.to raise_error( Admin::ProductSavingService::NotSavedProductError) # custom error
				end

				it "set validation :errors" do
			...
			context "with invalid :productable params" do
				...
				it "keep old categories" do
					service = error_proof_call( game_params, product )
					product.reload
					expect(product.category_ids).to contain_exactly *old_categories.map(&:id)
				end
			end
		end
		...
		context "without loaded product" do
			...
			context "with valid params" do
				...
				it "sets created product" do
					service = described_class.new(params)
					service.call
					expect(service.product).to be_kind_of(Product) # be_kind_of is new 
				end
			...
			context "with invalid :productable params" do
				...
				it "does not create a category association" do
					expect{
						error_proof_call(params)
					}.to_not change(ProductCategory, :count)
				end
...
# this method is apresentation more information about error

	...
	context 'GET /categories' do
		...
		context "with search[name] param" do
			...
			it "returns only searched categories limited by default pagination" do
				...
				expected_categories = search_name_categories[0..9].map { |category| category.as_json(only: %i(id name)) }
				expect(body_json['categories']).to contain_exactly *expected_categories
			end
		...
		context "with pagination params" do
			...
			it "returns categories limit by pagination" do
				get url, headers: auth_header(user), params: pagination_params
		...
		context "with order params" do
			...
			it "returns ordered categories limit by default pagination" do

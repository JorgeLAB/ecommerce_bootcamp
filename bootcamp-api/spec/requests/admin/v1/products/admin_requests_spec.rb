RSpec.describe "Admin V1 Product as :admin", type: :request do
	...
	context "GET /products" do
		...
		context "with any params" do
			...
			it "returns Products with :productable following default pagination" do
				...
				expect(body_json['product']).to contain_exactly *expected_return
			end
		...
		context "with search[name] params" do
			...
			let(:search_params) { {name: "Search"} }

			it "returns only searched products limited by default pagination" do
				get url, headers: auth_header(user), params: search_params
				expected_return = search_name_product.map do |product| 
					build_game_product_json(product)
				end
		...
		context "with order params" do
			...
			it "returns ordered products limited by default pagination" do
				...
				expect(body_json['product']).to contain_exactly *expect_return
			end

			it "return success status" do
				get url, headers: auth_header(user), params: order_params
				expect(resposne).to have_http_status(:ok)
			end
		end
	end
	...
	context "POST /products" do
		...
		let(:post_header) { auth_header(user, merge_with: {'Content-Type' => 'multipart/form-data' } ) } # or base 64

		context "with valid params" do
			let(:game_params) { attributes_for(:game, system_requirement: system_requirement.id) }
			...
			it "returns last added Product" do
				post url, headers: post_header, params: product_params
				expected_return = build_game_product_json(Product.last)
				expect(body_json['product']).to contain_exactly *expect_return 
			end
		end
		...
		context "with invalid :productable params" do
			let(:game_params){ attributes_for(:game, developer: "", system_requirement: system_requirement.id) }
	...
	context "PATCH /products/:id" do
		...
		context 'with invalid Product params' do
			...
			it 'does not updates Product' do
				...
				product.reload
				expect(product.name).to old_product_name 
			end
	...
	context "DELETE /products/:id" do
		...
		it 'removes all associated product categories' do
			product_categories = create_list(:product_categories, 3, product: product)
			delete url, headers: auth_header(user)
...
def build_game_product_json(product)
	...
	json['categories'] = product.categories.map(&:id)
	json['image_url'] = rails_blob_url(product.image)
	json['productable'] = product.productable_type.underscore # why transpalein snake_case

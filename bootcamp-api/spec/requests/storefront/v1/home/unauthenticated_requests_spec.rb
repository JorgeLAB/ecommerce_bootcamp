  ...
  context 'GET /home' do
    ...
    context 'on :featured' do
      ...
      it 'returns random featured products' do
        get url, headers: unauthenticated_header
        expected_products = featured_products.map { |product| build_game_products_json(product) }
      ...
      it 'does not returns any non_featured products' do
        get url, headers: unauthenticated_header
    ...
    context 'on :last_realeses' do
      ...
      it 'return random last released products' do
        get url, headers: unauthenticated_header
    ...
    context 'on :cheapest' do
      ...
      it 'returns cheapest products' do
        ...
        expected_products = price_ordered_products.map { |product| build_game_products_json(product) }

        expect(body_json['cheapest']).to contain_exactly *expected_products.take(4)
      end

      it 'does not return anny non-last released product' do
        get url, headers: unauthenticated_header

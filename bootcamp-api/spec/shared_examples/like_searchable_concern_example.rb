shared_examples "like searchable concern" do | factory, field |
	let!(:search_param) { 'Example' }
	let!(:records_to_find) do
		(0..3).to_a.map { |index| create(factory, field => "Example #{index}") }
	end
	let!(:records_to_ignore){ create_list(factory, 3) }
